...
(() => {
  ...
  const apply = (mode, {fromStorage=false} = {}) => {
    ...
    safeSet(btn, 'aria-pressed', String(nextMode === 'dark'));
    safeSet(btn, 'data-mode', nextMode);
    safeSet(btn, 'title', `Theme: ${label}`);
    safeSet(btn, 'aria-label', `Theme: ${label}. Click to change theme.`);
    if (btn) btn.textContent = label;
    if (!fromStorage) setStored(nextMode);
  };
...
function initHero(){
  ...
  const tabs = Array.from(document.querySelectorAll('#hero .hero-captions [data-emotion]'));
  const texts = Array.from(document.querySelectorAll('#hero .hero-caption-texts [data-emotion]'));

  function setHeroFrame(index){
    if(!imgs.length) return;
    current = ((index % imgs.length) + imgs.length) % imgs.length;
    imgs.forEach((img, ix) => img.classList.toggle('active', ix === current));
    tabs.forEach((tab, ix) => {
      const active = ix === current;
      tab.classList.toggle('active', active);
      tab.setAttribute('aria-selected', String(active));
    });
    texts.forEach((text, ix) => { text.hidden = ix !== current; });
  }

  function advance(){
    setHeroFrame(current + 1);
  }

  tabs.forEach((tab, ix) => {
    tab.addEventListener('click', () => setHeroFrame(ix));
  });
...
// initBarPlot('my-bar');      // or another container id
  ...
  stepCallbacks.set('media-types', (state)=>{
    const active = state === 'enter';
    stackedEl?.classList.toggle('is-active', active);
    document.getElementById('dot-plot')?.classList.toggle('is-active', active);
  });
...
function initStoryUI(){
  ...
  const storySections = Array.from(document.querySelectorAll('#testing-main section[id]'));
  ...
  storySections.forEach(section => navObserver.observe(section));

  const panelCopy = {
    pubs: {
      all: '<strong>Takeaway:</strong> The timeline shows slow accumulation until the field accelerates in the 2020s.',
      recent: '<strong>Takeaway:</strong> 63 of 124 publications appear from 2020 to 2024, so the latest platform shifts matter a lot.'
    },
    media: {
      overview: '<strong>Takeaway:</strong> Traditional media remains the largest research focus, which makes long-running newspaper and broadcast evidence easier to compare.',
      social: '<strong>Takeaway:</strong> Twitter/X and Instagram dominate social-media studies, while TikTok and Reddit are still thinly represented.'
    },
    modalities: {
      unimodal: '<strong>Takeaway:</strong> Still images are the most common unit of analysis, making them the field&rsquo;s clearest baseline.',
      multimodal: '<strong>Takeaway:</strong> Image-plus-text studies are common, but richer combinations with video and audio remain rare.'
    },
    countries: {
      rank: '<strong>Takeaway:</strong> Ranking the countries makes the US, UK, and Germany concentration easy to see.',
      map: '<strong>Takeaway:</strong> The map gives a broader view of reach, including countries that appear only once or twice.'
    },
    challenge: {
      access: '<strong>Next step:</strong> Document API limits and platform sampling bias so future reviews can compare evidence quality.',
      methods: '<strong>Next step:</strong> Pair close reading with computational approaches that are transparent, multilingual, and validated.',
      misinfo: '<strong>Next step:</strong> Study visual attacks on climate solutions, not only denial of climate science.',
      global: '<strong>Next step:</strong> Build more participatory research with Global South and Indigenous communities.'
    }
  };
  ...
  function updatePanel(target, key){
    document.querySelectorAll(`[data-insight-panel="${target}"]`).forEach(panel => {
      panel.classList.toggle('active', panel.dataset.panelKey === key);
    });

    const ids = chartFocus[target]?.[key];
    if (ids) {
      Object.values(chartFocus[target]).flat().forEach(id => {
        document.getElementById(id)?.classList.toggle('is-emphasized', ids.includes(id));
      });
    }
  }

  document.querySelectorAll('[data-panel-target][data-panel-key]').forEach(button => {
    const target = button.dataset.panelTarget;
    const key = button.dataset.panelKey;
    const group = Array.from(document.querySelectorAll(`[data-panel-target="${target}"]`));
